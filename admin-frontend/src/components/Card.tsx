...
export default function Card({ event, vendorUserNames }: CardProps) {
    ...
    const handleEventSubmit = async (updatedEventData: any) => {
        ...
        const updatedEvent: Event = {
            ...
            numberOfGuest: parseInt(updatedEventData.numberOfGuest, 10),
            shortDescription: updatedEventData.shortDescription,
            image: updatedEventData.image,
            address: updatedEventData.address,
            isBlocked: event.isBlocked,
            userName: updatedEventData.userName,
        };
        await AdminService.getUser(updatedEvent.userName!).catch((error) => {
            console.error("Error validating user:", error);
            setError("User validation failed. Please ensure the username exists.");
            throw error; // Re-throw to prevent further execution
        });
        console.log("Submitting updated event data:", updatedEvent);
        console.log("Is event id a number: " + typeof Number(event.eventId));
        await AdminService.updateEvent(Number(event.eventId), updatedEvent);
        setEventForm(null); // Close the event form modal after submission
        fetchEvents(); // Fetch all events again to update the list
    }
